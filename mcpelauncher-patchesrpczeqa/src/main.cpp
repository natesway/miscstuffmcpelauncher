...
extern "C" void __attribute__ ((visibility ("default"))) mod_preinit() {
    ...
    mcpelauncher_preinithook("getaddrinfo", (void*)+[](const char *node, const char *service, const addrinfo *hints, addrinfo **res) -> int {
        if (node) {
            std::string server = "";
            if (lastWasIp) {
                if (strcmp(node, "geo.hivebedrock.network") == 0 || strcmp(node, "na.zeqa.net") == 0 || strcmp(node, "eu.zeqa.net") == 0 || strcmp(node, "as.zeqa.net") == 0 || strcmp(node, "au.zeqa.net") == 0) {
                    patchId++;
                    std::thread([=]() {
                ...
                } else if (strcmp(node, "na.zeqa.net") == 0 || strcmp(node, "eu.zeqa.net") == 0 || strcmp(node, "as.zeqa.net") == 0 || strcmp(node, "au.zeqa.net") == 0) {
                    server = "Zeqa";
                }
